...
#   bge-reranker-v2-m3      568M   15.048 ms   θέση σωστού chunk: διάμεσος 1.0
#   ms-marco-MiniLM-L-6-v2   22M      693 ms   διάμεσος 1.0
#   -> 21.7x ταχύτερο, ΙΣΟΠΑΛΙΑ στην ποιότητα (7 καλύτερα / 7 χειρότερα / 30 ίδια)
...
# ΤΟ ΚΑΤΩΦΛΙ ΕΙΝΑΙ ΔΕΜΕΝΟ ΜΕ ΤΟ RERANKER_MODEL. Άλλο μοντέλο = άλλη κλίμακα
# σκορ. Αν αλλάξεις μοντέλο, ΞΑΝΑΜΕΤΡΑ με _reranker_compare.py — αλλιώς το gate
# θα κόβει σωστές απαντήσεις ή θα περνά άσχετες, σιωπηλά.
#   bge-reranker-v2-m3      sigmoid [0,1]      in-min  0.069 / out-max  0.013 -> 0.05
#   ms-marco-MiniLM-L-6-v2  ωμά logits ~[-11,+11]  in-min -1.797 / out-max -2.685
# Το -2.0 είναι το ΑΥΣΤΗΡΟΤΕΡΟ κατώφλι που δεν κόβει καμία σωστή (το q027, ο
# πίνακας αποθήκευσης, είναι στο -1.797) και αφήνει 0.685 περιθώριο πάνω από το
# χειρότερο out-of-corpus. Το probe πρότεινε -2.241 (μέσο του διάκενου)· προτιμάμε
# ασύμμετρο περιθώριο υπέρ της άρνησης, γιατί μια λάθος απάντηση σε ερώτηση εκτός
...
# ΤΙ ΛΥΝΕΙ (μετρημένο, probe_corrective_rewrite.py σε 16 σκόπιμα κακοδιατυπωμένες
# ερωτήσεις με ΗΔΗ επαληθευμένα keywords): όταν ο χρήστης ρωτά χωρίς την ορολογία
# των papers («τι λένε για τα νομικά ζητήματα;»), ο αγγλικός reranker δεν βρίσκει
# λεξιλογική επικάλυψη και το gate κόβει ερώτηση που ΕΙΧΕ απάντηση. 10/16 κόβονταν.
#
# ΓΙΑΤΙ ΞΕΧΩΡΙΣΤΟ, ΑΥΣΤΗΡΟΤΕΡΟ ΚΑΤΩΦΛΙ: το ερώτημα του 2ου pass το έγραψε το ΙΔΙΟ
# το σύστημα, όχι ο χρήστης -> δικαιούται λιγότερη εμπιστοσύνη. Χωρίς αυτό
# μετρήθηκαν 2 περιπτώσεις όπου το rewrite πέρασε το gate «πασπαλίζοντας» όρους
# του πεδίου ΧΩΡΙΣ να ανακτήσει το σωστό υλικό — δηλαδή ο agent ΠΑΡΑΚΑΜΠΤΕ την
# άμυνα αντί να τη βοηθά. Σάρωση κατωφλίου (πραγματικές/ψευδαισθήσεις):
#   -2.0 -> 5/2 · 0.0 -> 4/1 · +1.0 -> 4/0 · +1.5 -> 3/0
# Τα 5 out_of_corpus μένουν κομμένα σε ΟΛΑ τα κατώφλια.
#
# ΠΡΟΣΟΧΗ: το +1.0 είναι βαθμονομημένο σε n=10 ΧΩΡΙΣ validation set — το πιο
# αδύναμο σημείο της αλυσίδας. Αν μεγαλώσει το golden set, ΞΑΝΑΜΕΤΡΑ.
ENABLE_CORRECTIVE = os.getenv("ENABLE_CORRECTIVE", "1") not in ("0", "false", "False")
CORRECTIVE_MIN_SCORE = float(os.getenv("CORRECTIVE_MIN_SCORE", "1.0"))
