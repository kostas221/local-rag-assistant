...
| **`chunk_size` 750 / 1000** (10/8/2026, ξανά — η επιλογή του 2026 ήταν σε ΑΛΛΟ σύστημα) | Το `chunk_experiment.py` με **αναπαραγόμενο control** (το 1500 έδωσε 0.793/0.807/98.5% και ανά κατηγορία 0.869/0.751/0.831/0.542 — ταυτόσημα με τα καταγεγραμμένα). **750 scaled**: MRR −0.077 ΚΑΙ +25% latency (30 ζεύγη στον reranker αντί 15). **1000 scaled**: cov +7.3pp στο hard set (≈1 ερώτηση σε n=16) αλλά +25% latency ≈ +330 ms — ίδιο κόστος με το L-12, μηδέν όφελος. **1000 fixed-depths** (η μόνη εκδοχή που θα κέρδιζε latency): **μηδέν κέρδος, 0.8 vs 0.8** — το padding του `RERANK_BATCH_SIZE=4` κυριαρχείται από το μεγαλύτερο chunk του batch. Και **MRR +0.029 ΕΝΩ coverage −5.2pp**: το keyword-proxy MRR ανεβαίνει *μηχανικά* με μικρότερα chunks (μεγαλύτερη πυκνότητα keyword ανά chunk), ενώ το coverage δείχνει ότι 2-3 ερωτήσεις **έχασαν υλικό** — 622 chunks αλλά ακόμα top-30, άρα λιγότερο κείμενο. **Πέμπτη επιβεβαίωση ότι το MRR δεν είναι ο κριτής** |
...
- ~~chunk_size=1500 είναι κληρονομιά του v1~~ **ΕΠΙΚΥΡΩΘΗΚΕ 10/8/2026** σε 2 σετ × 2 μεθόδους βαθών, με αναπαραγόμενο control· βλ. ΑΠΟΡΡΙΦΘΗΚΑΝ
